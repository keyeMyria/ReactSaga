...
import styles, { btnStyle, sheetStyle, hairlineWidth } from '../Styles/ActionSheetCustomStyles';
...
export class ActionSheet extends PureComponent {
  ...
  _hideSheet = (callback) => {
    Animated.timing(this.state.sheetAnim, {
      ...
      duration: 150
    }).start(callback || this.emptyCallBack);
  };
  ...
  _createButton = (title, fontColor, index, style) => {
    ...
    if (React.isValidElement(title)) {
      ...
      titleNode = (
        <Text allowFontScaling={false} style={[btnStyle.title, { color: fontColor }]}>
          {title}
        </Text>
      );
    }

...
function getPath(uri) {
  ...
  return `${RNFetchBlob.fs.dirs.CacheDir}/openmed-media-cache${url.pathname}`;
}
...
export class CachedImage extends Component {
  ...
  componentWillMount() {
    ...
    // base64 image, local image or image assets
    // eslint-disable-next-line
    if (source && ((source.uri && source.uri.indexOf('http') < 0 && source.uri.indexOf('https') < 0) || !source.uri)) {
